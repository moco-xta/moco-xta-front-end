...
vec4 rgbShift(sampler2D uTexture, vec2 vUv, float shift) {
   float r = texture2D(uTexture, vUv + vec2(shift * 0.05, 0.0)).r;
   float g = texture2D(uTexture, vUv).g;
   float b = texture2D(uTexture, vUv + vec2(shift * 0.05, 0.0)).b;
...
void main() {
	...
	vec4 color = rgbShift(uTexture, newUv, shift);
