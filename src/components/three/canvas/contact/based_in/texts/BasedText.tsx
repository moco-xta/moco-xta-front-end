...
import {
  getBasedTextAnimationsData,
  getBasedTextDefaultValues,
} from '@/data/contact/three/based_in/texts/basedTextData'

import { animate } from '@/animation/index'

export default function BasedText() {
  const t = useTranslations('CONTACT')
  const { timeline } = useGSAPTimelineContext()
  const { textSplitted, textGroupRef, textLengthRef } = useSplitted3DText(t('BASED_IN.BASED'))
  ...
  useGSAP(() => {
    ...
    contactLetters.forEach((letterRef) => {
      animate({
        ...
        animationsData: getBasedTextAnimationsData(),
      })
    })
  })
