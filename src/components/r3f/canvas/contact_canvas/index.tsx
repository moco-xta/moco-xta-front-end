...
function ContactScene() {
  ...
  useFrame(() => {
    // @ts-ignore
    if(ref.current) ref.current.distort = THREE.MathUtils.lerp(ref.current.distort, hovered ? 0.4 : 0, hovered ? 0.05 : 0.01)
  })
  ...
  return (
    <>
      ...
      <directionalLight
        ...
        castShadow
      />
      <mesh ref={meshRef} onPointerOver={() => hover(true)} onPointerOut={() => hover(false)} position={[-1.7, 0, 0]} scale={[3, 4, 1]}>
        <planeGeometry args={[1, 1, 32, 32]} />
        <MeshDistortMaterial ref={ref} speed={5} map={colorMap} transparent>
