...
void main()	{
	...
	if(uType == 0){
		...
		float r = texture2D(uDiffuse, newUV.xy += c * (uVelocity * .5)).x;
		float g = texture2D(uDiffuse, newUV.xy += c * (uVelocity * .525)).y;
		float b = texture2D(uDiffuse, newUV.xy += c * (uVelocity * .55)).z;
		color = vec4(r, g, b, 1.);
	}
	...
	if(uType==1){
		...
		color = texture2D(uDiffuse,warpedUV) + texture2D(uDiffuse,warpedUV)*vec4(vec3(c),1.);
	}
